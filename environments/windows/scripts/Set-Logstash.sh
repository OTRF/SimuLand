...
if [ -z "$EVENTHUB_NAMESPACE" ] || [ -z "$EVENTHUB_CONNECTIONSTRING" ] || [ -z "$EVENTHUB_NAME" ] || [ -z "$LOCAL_USER" ]; then
  usage
    ...
    wget -O /opt/logstash/scripts/logstash-entrypoint.sh https://raw.githubusercontent.com/OTRF/mordor-labs/master/environments/windows/logstash/scripts/logstash-entrypoint.sh
    wget -O /opt/logstash/pipeline/eventhub.conf https://raw.githubusercontent.com/OTRF/mordor-labs/master/environments/windows/logstash/pipeline/eventhub.conf
    wget -O /opt/logstash/config/logstash.yml https://raw.githubusercontent.com/OTRF/mordor-labs/master/environments/windows/logstash/config/logstash.yml
    wget -O /opt/logstash/docker-compose.yml https://raw.githubusercontent.com/OTRF/mordor-labs/master/environments/windows/logstash/docker-compose.yml
    wget -O /opt/logstash/Dockerfile https://raw.githubusercontent.com/OTRF/mordor-labs/master/environments/windows/logstash/Dockerfile
